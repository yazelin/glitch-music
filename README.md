...
| 本機曲庫 | 「加入歌曲」或拖放：mp3／m4a／**mp4（取音軌）**／flac／ogg／wav，存進瀏覽器 IndexedDB（不上傳雲端）。mp3 讀 ID3（v2.2／v2.3／v2.4、unsynchronisation、檔尾 ID3v1、Big5 標成 latin1 的老檔）、mp4/m4a/m4v 讀 ilst atoms（含 moov>meta 變體），標題／演出者／專輯／封面都會帶出；解析器升級時啟動會自動補掃舊歌的標籤，不用重新匯入。內建曲可從歌單移除，但會留在曲庫（它來自 tracks.json，刪了下次也會回來） |
