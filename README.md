...
| 電台分頁 | 電台完全獨立於曲庫／歌單／佇列：自己的「電台」分頁，選一台就一直聽（不輪播、上一首下一首停用），轉台＝回電台頁點別台，點任何歌即退出電台模式。內建預設台（**警廣全國治安交通網＋臺北／臺中／高雄臺**、飛碟電台、SomaFM×2、Radio Paradise、LISTEN.moe，皆為官方公開串流；警廣是 HLS——Safari/iOS/Android 原生播，其他瀏覽器自動載入自架的 hls.js）只是預設值——可刪、可用「＋ 加電台」加自己的串流網址。播放顯示 LIVE；跨站音訊走第二顆 audio 元素、不接頻譜 graph（沒 CORS 的來源才不會被靜音），頻譜以模擬動畫呈現 |
