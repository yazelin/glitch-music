...
| 續播記憶 | 重開 app 自動選回上次的歌單、歌曲與**播放位置**（近曲尾自動歸零從頭）；電台模式不記位置 |
| 主題配色 | 右上角一鍵循環四款：暗／淺 × glitch（藍綠紫）／補色（粉橘萊姆）。整站顏色走 design token，頻譜 canvas 的色相與混色模式跟著主題換（淺色底不能用加法發光），選擇會記住 |
| 頻譜視覺 | 桌面接 Web Audio 真頻譜；**行動裝置(iOS 與 Android)一律不接 graph**——鎖屏/螢幕關閉時系統會掛起 AudioContext、聲音跟著斷,背景播放優先於視覺,頻譜用時間函數模擬動畫 |
...
| 分享曲留下來 | 分享進來的歌多兩顆按鈕：「存進曲庫」把音檔抓下來存進 IndexedDB（連封面與歌詞一起），之後跟自己匯入的歌一樣永久保存、離線可播、進得了歌單；「下載」把 mp3 存成檔案。分享連結會活得比音檔久（來源到期就清），存起來才是真的留住 |
| 分享連結 | 網址帶 `?id=<code>` 進來時，向短網址服務（`shorturl.yazelinj303.workers.dev`）換回那首歌的曲名、音檔網址與歌詞，插進曲庫最前面直接播、並自動打開歌詞面板。音檔本身不進資料庫，`src` 是它原本的網址——跨站 URL 由播放器判定走第二顆 audio 元素、不接頻譜 graph，所以來源不必開 CORS。自動播放被瀏覽器擋掉時安靜退回待播狀態，不跳錯誤 |
...
- Android Chrome／已安裝 PWA：`<audio>` 在螢幕關閉後繼續播放，鎖屏有控制列。**前提是聲音不經 Web Audio graph**——螢幕關閉時 AudioContext 會被掛起，所以行動裝置一律走純 `<audio>`。回前景若發現被系統暫停會自動續播。
